...
import { VoteBank } from '../target/types/vote_bank';

describe("onchain-voting", () => {
  anchor.setProvider(anchor.AnchorProvider.env());
  const program = anchor.workspace.onchain_voting as Program<OnchainVoting>;
  ...
  it("Vote for GM", async () => {
    ...
    let voteBankData: VoteBank = await program.account.VoteBank.fetch(voteBank.publicKey);
